...
app.post("/upload", async (c) => {
  try {
    ...
    if (!files || files.length === 0 || !tableName) {
      return c.json(
        { message: "No files or table name provided. Please upload valid files and specify the table name." },
        400
      );
    }
    ...
    __name(compressData, "compressData");
    for (const file of files) {
      let inferDataType = /* @__PURE__ */ __name(function(key, value) {
        if (key === "Shapes") {
          return "BLOB";
      ...
      try {
        ...
        const cleanJsonData = fileContent.replace(/^\uFEFF/, "");
        jsonData = await JSON.parse(cleanJsonData);
        console.log("jsonData", jsonData);
      } catch (err) {
        success = false;
        console.error(`Error reading or parsing file ${file.name}: ${err.message}`);
        continue;
      }
      ...
      if (!keys.includes("EQID")) {
        success = false;
        logData.push(`EQID' key is required for the primary key in file ${file.name}.`);
        console.error(`'EQID' key is required for the primary key in file ${file.name}.`);
        continue;
      }
      const createTableSQL = `
        CREATE TABLE IF NOT EXISTS ${tableName} (
          ${keys.map((key) => `${key} ${inferDataType(key, records[0][key])}`).join(", ")},
          PRIMARY KEY (EQID)
        );
      `;
      try {
        ...
        success = false;
        logData.push(`Error creating table for file ${file.name}: ${err.message}`);
        console.error(`Error creating table for file ${file.name}: ${err.message}`);
        continue;
      }
      let startIndex = 0;
      const batchSize = 10;
      while (startIndex < records.length) {
        logData.push(`Processing Batch start index ${startIndex} `);
        console.log(`Processing Batch start index ${startIndex} `);
        const batch = await Promise.all(
          records.slice(startIndex, startIndex + batchSize).map(async (record) => {
            if (record.Shapes) {
              try {
                console.log("Original Shapes before compression:", record.Shapes);
                record.Shapes = await compressData(record.Shapes);
                console.log("Compressed Shapes (ArrayBuffer) :", record.Shapes);
              } catch (err) {
                console.error(`Error compressing Shapes column: ${err.message}`);
                throw err;
              }
            }
            return record;
        ...
        const insertValues = batch.map((record) => `(${keys.map((key) => `'${sanitize(record[key])}'`).join(", ")})`).join(", ");
        const insertSQL = `
        INSERT INTO ${tableName} (${keys.join(", ")})
        VALUES ${insertValues}
        ON CONFLICT(EQID) DO UPDATE SET ${keys.filter((key) => key !== "EQID").map((key) => `${key} = excluded.${key}`).join(", ")};
      `;
        console.log("insertSQL=>", insertSQL);
        try {
          await c.env.DB.prepare(insertSQL).run();
          logData.push(`query exucte of batch stared from ${startIndex}`, insertSQL);
        } catch (err) {
          logData.push("uploading error : ", err);
          console.log("uploading error : ", err);
          success = false;
          logData.push(`Error inserting batch at index ${startIndex} for file ${file.name}:`, err.message);
          console.error(`Error inserting batch at index ${startIndex} for file ${file.name}:`, err.message);
        }
        startIndex += batchSize;
      }
    }
    if (success) {
      return c.json({ message: "All files successfully uploaded and processed.", logData }, 200);
    } else {
      return c.json({ message: "Error processing some files.", logData }, 500);
    }
  } catch (err) {
...
app.post("/manufacturer", async (c) => {
  try {
    ...
    if (!search) {
      const sqlQuery = `SELECT DISTINCT Manufacturer FROM search2 ORDER BY Manufacturer ASC`;
      console.log("SQL Query for all data (search missing):", sqlQuery);
      const queryResult = await c.env.DB.prepare(sqlQuery).all();
      const results2 = queryResult?.results || [];
      if (results2.length === 0) {
        return c.json({ manufacturers: [], totalManufacturer: 0 }, 200);
      }
      const manufacturers2 = results2.map((row) => row.Manufacturer);
      return c.json({ manufacturers: manufacturers2, totalManufacturer: manufacturers2.length }, 200);
    }
    ...
    for (const strategy of queryStrategies) {
      const sqlConditions = words.map((word) => `
      ...
      const sqlQuery = `SELECT DISTINCT Manufacturer FROM search2 WHERE ${sqlCondition} ORDER BY Manufacturer ASC`;
...
app.post("/equipment", async (c) => {
  try {
    ...
    console.log("Request Body equipment:", { Manufacturer, keywords, condition });
    if (!Manufacturer) {
    ...
    for (const strategy of queryStrategies) {
      ...
      const sqlQuery = `
        ...
        ORDER BY EQType ASC
      `;
      console.log("SQL Query with strategy:", sqlQuery);
      const queryResult = await c.env.DB.prepare(sqlQuery).bind(Manufacturer).all();
      results = queryResult?.results || [];
      if (results.length > 0) {
        return c.json(results, 200);
        ...
        const keywordConditions = words.map(
          (word) => `
            (EQID LIKE '${strategy(word)}' OR
             ...
             MfgProdNo LIKE '${strategy(word)}' OR 
             MfgDesc LIKE '${strategy(word)}' OR
             JSON_EXTRACT(Views, '$[0].ShapeID') LIKE '${strategy(word)}')`
        );
        const combinedConditions = keywordConditions.join(` ${validCondition} `);
        const fullQuery = `${sqlQuery} AND (${combinedConditions})`;
        console.log("Executing SQL Query with strategy:", fullQuery);
        console.log("With Parameters:", queryParams);
        const queryResult2 = await c.env.DB.prepare(fullQuery).bind(...queryParams).all();
        results = queryResult2?.results || [];
        if (results.length > 0) {
          return c.json(results, 200);
        }
      }
...
async function decompressData(compressedArrayBuffer) {
  try {
    ...
    await writer.close();
    const decompressedArrayBuffer = await new Response(stream.readable).arrayBuffer();
...
__name(decompressData, "decompressData");
app.get("/", async (c) => {
  try {
    const { results } = await c.env.DB.prepare("SELECT Shapes FROM m1 LIMIT 1").all();
    if (!results.length) {
      return c.json({ message: "No data found" }, 404);
    }
    let compressedData = results[0].Shapes;
    if (typeof compressedData === "string") {
      console.error("Error: Shapes data is a string, expected BLOB");
      return c.json({ message: "Invalid data format in database" }, 500);
    }
    const decompressedData = await decompressData(compressedData);
    if (!decompressedData) {
      return c.json({ message: "Failed to decompress data" }, 500);
    }
    return c.json({ decompressedData: JSON.parse(decompressedData) });
  } catch (err) {
...
// .wrangler/tmp/bundle-BMCCHf/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-BMCCHf/middleware-loader.entry.ts
