...
app.get("/", async (c) => {
  try {
    const result = await c.env.DB.prepare("SELECT DISTINCT Manufacturer FROM search2").all();
    return c.json({ success: true, result });
  } catch (error3) {
    return c.json({ success: false, error: error3.message });
  }
});
var src_default = app;
...
// .wrangler/tmp/bundle-D0BgVD/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-D0BgVD/middleware-loader.entry.ts
