    ...
    return (
        <div className={`nz-searchControl ${!searchControlProps.isShowFilterControl ? 'nz-hidefilter-icon' : ''}`} key={searchControlProps.uniqueName}>
            ...
            <div className='nz-search-control'>
                <EditTextControl {...searchControl} allowInputChangeEvent={true} value={searchControlProps.searchInputValue} isRequired={false} handleValueChange={searchControlProps.searchValueChange} />
            </div>
            <div className='nz-lens-icon'>
                <DirtyFlagImage {...lensIcon} bgColor={`${searchControlProps.lensDirty&&"#7BF1A8"}`} handleMouse={searchControlProps.handleLensMouse} isDirty={searchControlProps.lensDirty}  />
            </div>
        </div>
    )
}
