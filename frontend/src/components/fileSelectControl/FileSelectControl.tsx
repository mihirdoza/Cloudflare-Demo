...
import "./style/FileSelectControl.css"; //'../../className/FileSelectControl.css'
import Label from "../psrLabel/Label";

export const FileSelectControl = (
  fileSelectControlProps: IfileSelectControl
) => {
  ...
  useEffect(() => {
    ...
    if (fileSelectControlProps.value && typeof fileSelectControlProps.value === 'string') {
      ...
      if (reader.result) {
        toBase64(file).then((result: any) => {
          if (result) {
            ...
            fileSelectControlProps.handleValueChange &&
              fileSelectControlProps.handleValueChange(file_name, "FPFileName");
            fileSelectControlProps.handleValueChange &&
              fileSelectControlProps.handleValueChange(
                file.name,
                fileSelectControlProps.uniqueName
  ...
  return (
    <div
      key={fileSelectControlProps.uniqueName}
      className="form-control-labeled"
    >
      {fileSelectControlProps.isRenderAsForm && (
        <Label
          uniqueName={`${fileSelectControlProps.uniqueName}-label`}
          tooltip={
            fileSelectControlProps.nameDesc
              ? fileSelectControlProps.nameDesc
              : ""
          }
          label={`${fileSelectControlProps.label || ""}${
            fileSelectControlProps.isRequired ? " (Required)" : ""
          }`}
        />
      )}
      <BaseFileSelect
        ...
        label={
          fileSelectControlProps.label || fileSelectControlProps.uniqueName
        }
        onChangeFile={onChangeFile}
      />
      {fileSelectControlProps.isRenderAsForm &&
        fileSelectControlProps.valueDesc && (
          <Label
            uniqueName={`${fileSelectControlProps.uniqueName}-desc`}
            label={fileSelectControlProps.valueDesc}
            styleClasses="italic font-[8px]"
          />
        )}
    </div>
  );
