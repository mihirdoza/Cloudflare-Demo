...
import ComboboxControl from "../../components/DisplayControls/ComboBoxControl/ComboBoxControl";
import { searchProps } from '../../SampleDataset/TestDirtyFlag';
import SearchControl from "../../components/SearchControl/SearchControl";
import SearchResultsModal from "../../components/DeviceGridModal/DeviceGridModal";
import { ToggleControl } from "../../components/DisplayControls/ToggleControl/ToggleControl";
import filterByAttribute from "../../JsonData/filterByAttribute.json";
import { SearchDatacolDef } from "./SearchUiConfig";
...
const SearchDatabase: React.FC = () => {
  ...
  const [condition, setCondition] = useState<any>("AND");
  ...
  const Header={ "Authorization": `Bearer ${import.meta.env.VITE_API_TOKEN}`,  // Cloudflare API Token
  ...
  //  Show All Slected Search Result
  ...
  //  Shoe Equipment Base on Manufacturer Select
      ...
      <div className="home-main" >
        ...
        <div className="main-manufacturer">
          ...
          <div className="d-flex justify-content-between align-items-center ">
            ...
            <Button onClick={() => { navigate("/") }} variant={'contained'} disabled={loading} className={`upload-btn ${loading ? "bg-button-loading" : "bg-button"}`}>
          ...
          <ToggleControl uniqueName={"Include Related"} isRenderAsForm={true} value={`${isRelated}`} label={"Include Related"} handleValueChange={HandleRelatedValue} ></ToggleControl>
          <ComboboxControl isRenderAsForm={false} uniqueName='Manufacturer' label={`Manufacturer  (${manufacturers.length})`} value={selectedManufacturer} optionsData={manufacturers?.length > 0 ? manufacturers : []} type='string' handleValueChange={(value: any, action: any) => {
            console.log("Combobox changed:", value, action);
            setSelectedManufacturer(value)
          }} nameDesc={`Manufacturers`}   ></ComboboxControl>
          <ComboboxControl uniqueName='Equipment types' label={`Equipment Types (${equipmentType.length})`} value={selectedEquipmentType} optionsData={equipmentType} type='string' handleValueChange={(value: any, action: any) => {
            console.log("Combobox changed:", value, action);
            setSelectedEquipmentType(value)
          }} nameDesc={`EquipMent Types`}></ComboboxControl>
          <ComboboxControl uniqueName='Product line' label={`Product Line (${productLine.length})`} value={selectedProductLine} optionsData={productLine} type='string' handleValueChange={(value: any, action: any) => {
            console.log("Combobox changed:", value, action);
            setSelectedProductLine(value)
          }} nameDesc={`Product Line`}></ComboboxControl>
          <ComboboxControl uniqueName='Product number' label={`Product Number (${productNumber.length})`} value={selectedProductNumber} optionsData={productNumber} type='string' handleValueChange={(value: any, action: any) => {
            console.log("Combobox changed:", value, action);
            setSelectedProductNumber(value)

          }} nameDesc={`Product Number`}></ComboboxControl>

          <ComboboxControl uniqueName='Filter By Attribute' label={`Filter By Attribute (${filterByAttribute?.length})`} value={selectFBA} optionsData={filterByAttribute} type='string' handleValueChange={(value: any, action: any) => {
            console.log("Combobox changed:", value, action);
            setSelectFBA(value)
          ...
          <div className="d-flex justify-content-between align-items-center fbutton mt-1"  >
            {executionTime > 0 && <p>{"Time Taken - " + executionTime}</p>}
            <Button onClick={handleSearch} disabled={!selectedManufacturer || loading} variant={'contained'} className={`upload-btn ${loading || !selectedManufacturer ? "bg-button-loading" : "bg-button"}`} >
              <Label uniqueName={"Search"} label={"Search"}></Label>
            </Button>
          </div>
        </div>
        <SearchResultsModal Name="search_result" filterResult={filterResult} handleMouseEvent={(event: any, gridRef: any) => { console.log('') }} showModal={showModal} setShowModal={(val: any) => { setShowModal(val) }} colDef={SearchDatacolDef} ></SearchResultsModal>
        {loading && (
